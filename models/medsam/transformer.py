...
from models.medsam.common import MLPBlock, Adapter
...
class TwoWayTransformer(nn.Module):
    def __init__(
        ...
        decoder_adapt_depth: int = 2,
        if_mask_decoder_adapter: bool = True
    ) -> None:
        """
        ...
        self.decoder_adapt_depth = decoder_adapt_depth
        self.if_mask_decoder_adapter = if_mask_decoder_adapter

        for i in range(depth):
            if i < self.decoder_adapt_depth:
                if_adapter = self.if_mask_decoder_adapter
            else:
                if_adapter = False
            self.layers.append(
